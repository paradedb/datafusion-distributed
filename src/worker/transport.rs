...
pub trait PartitionSink: Send {
    ...
    /// this turns true, so a cancel doesn't just skip the send, it ends the upstream scan and
    /// drops the input stream, cascading the cancel further up. Default `false` for links that
    /// don't carry a cancel signal.
