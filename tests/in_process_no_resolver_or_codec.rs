...
mod tests {
    ...
    /// Under `in_process_mode = true`, neither `_annotate_plan`'s `max_tasks` computation
    /// nor `prepare_plan`'s `available_urls` lookup may require a registered `WorkerResolver`.
    /// We register only an in-process `WorkerTransport` stub, no resolver, no
    ...
    async fn in_process_mode_skips_resolver_and_codec() -> Result<()> {
        ...
        // Set the four knobs an in-process embedder needs, and crucially DO NOT register
        // a `WorkerResolver` or a user `PhysicalExtensionCodec`.
        ...
        // distributed planner converts into a `NetworkShuffleExec`. That's the boundary
        // that exercises both `_annotate_plan` and `prepare_plan`'s gates.
        let plan = ctx
        ...
        // minus the gRPC send tasks. With `in_process_mode = true` the gates short-circuit
        // both resolver lookups, so this call succeeds without one registered.
        distributed.prepare_in_process_plan(&ctx.task_ctx())?;

        Ok(())
    }

    /// Bare-minimum `WorkerTransport` impl. The test stops at `prepare_in_process_plan`,
    /// which only walks and rewrites the plan, so `open()` should never be reached. It
    /// would only fire if the in-process skip in the per-worker loop regressed.
