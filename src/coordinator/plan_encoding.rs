...
/// drift between transports: each task must see its own slice of task-isolated nodes, and the
/// worker decodes with the same codec stack the coordinator encoded with.
