...
use crate::{
    ProducerHead, RemoteStage, RemoteWorkUnitFeedTxs, WorkerConnection, WorkerDispatch,
    WorkerDispatchRequest, WorkerResolver, WorkerTransport,
};
...
impl WorkerConnection for ShmMqWorkerConnection {
    fn execute(&self, partition: usize) -> Result<BoxStream<'static, Result<RecordBatch>>> {
        ...
        // iteration checks for cancellation (via the injected interrupt extension point), drains the receiver
