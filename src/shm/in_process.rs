...
fn reinstantiate(plan: &Arc<dyn ExecutionPlan>) -> Arc<dyn ExecutionPlan> {
    ...
    if children.is_empty() {
        ...
        Arc::clone(plan)
            ...
            .expect("replace_children with the same arity")
    }
}
...
) -> Option<Result<crate::events::DesiredTaskCountEventResponse>> {
    let n_tasks = ev.session_config.target_partitions();
    MemShardConfig::mem_source(ev.plan).map(|_| {
        Ok(crate::events::DesiredTaskCountEventResponse::desired(
            n_tasks,
        ))
    })
}
...
mod tests {
    ...
    /// send path's compact-and-chunk has to carry the query; the result must still match the
    /// serial reference exactly.
    #[tokio::test(flavor = "current_thread")]
    async fn oversized_aggregate_frames_stream_across_tiny_rings() {
