...
/// offset math is a simple multiply.
#[inline]
pub(super) fn aligned_queue_bytes(queue_bytes: usize) -> usize {
    queue_bytes & !(MAXALIGN - 1)
}

/// MAXALIGN-UP `n` to the next multiple of [`MAXALIGN`]. Returns `None` on overflow. Used by
/// [`super::dsm::compute_dsm_layout`] to keep section boundaries MAXALIGN-aligned.
#[inline]
...
/// `&self` `try_recv` can hand the populated buffer back via `mem::take` without `RefCell` runtime
/// borrow tracking.
///
...
// touches it at a time, and `Mutex<Vec<u8>>` protects the scratch from shared-reference
// UB if that invariant ever slips. Send is auto-derived.
...
impl DsmInboxReceiver {
    ...
    /// Register the consumer's opaque wakeup token on the underlying ring. The embedder packs
    /// whatever its [`super::mpsc_ring::Wakeup`] interprets (a PG embedder packs `(pgprocno, pid)`).
    pub(super) fn set_receiver(&self, token: u64) {
        self.inner.set_receiver(token);
    }
}

impl BatchChannelReceiver for DsmInboxReceiver {
    fn try_recv(&self) -> RecvOutcome {
        let mut buf = self.scratch.lock().expect("scratch mutex poisoned");
...
mod tests {
    ...
    fn test_wakeup() -> std::sync::Arc<dyn Wakeup> {
