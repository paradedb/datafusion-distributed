...
mod tests {
    ...
    #[test]
    fn shuffle_reports_shuffle_kind() -> Result<()> {
        ...
        let exec = NetworkShuffleExec::try_new(input, 1)?;
    ...
    #[test]
    fn broadcast_reports_broadcast_kind() -> Result<()> {
        let input: Arc<dyn ExecutionPlan> = Arc::new(BroadcastExec::new(empty_with_field("a"), 1));
        let exec = NetworkBroadcastExec::try_new(input, 1)?;
    ...
    #[test]
    fn coalesce_reports_coalesce_kind() -> Result<()> {
        let exec = NetworkCoalesceExec::try_new(empty_with_field("a"), 1, 1)?;
