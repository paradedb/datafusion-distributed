...
/// Collects the per-node metrics of `plan` in pre-order traversal order, encoded as protos. A push
/// transport reuses this for its metrics frame so the per-node shape matches what the coordinator
/// channel reports.
