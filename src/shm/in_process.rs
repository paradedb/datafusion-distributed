...
//! change breaks the channel-protocol contract, the boundary routing API, or
...
fn collect_stages(
    ...
    if let Some(nb) = plan.as_ref().as_network_boundary() {
        ...
        let route_consumer_tasks = || {
            // The producer head fans its output into `per_task * consumer_task_count` partitions,
            // contiguous per consumer task, so producer output partition `q` is read by consumer
            // task `q / per_task`. That mirrors the `off = per_task * task_index` slicing both the
            // shuffle and broadcast boundaries apply on the consume side; `per_task` is the consumer
            // boundary's own per-task output partition count.
            let per_task = plan.output_partitioning().partition_count().max(1);
            let n_out = stage
                .local_plan()
                .map_or(0, |p| p.properties().partitioning.partition_count());
            (0..n_out)
                .map(|q| (q / per_task) as u32)
                .collect::<Vec<u32>>()
        };
...
mod tests {
    ...
    async fn in_process_distributed_query_matches_serial() {
        ...
        // from the mesh through ShmWorkerChannel::execute_task.
