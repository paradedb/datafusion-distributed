...
mod tests {
    ...
    async fn local_dynamic_filters() -> Result<()> {
        ...
        └──────────────────────────────────────────────────
          ...
          │   DistributedLeafExec:
          │     t0: DataSourceExec: file_groups={3 groups: [[/testdata/weather/result-000000.parquet:<int>..<int>], [/testdata/weather/result-000000.parquet:<int>..<int>, /testdata/weather/result-000001.parquet:<int>..<int>], [/testdata/weather/result-000002.parquet:<int>..<int>]]}, projection=[MinTemp], file_type=parquet, predicate=DynamicFilter [ MinTemp@0 IS NULL OR MinTemp@0 > <runtime> ], sort_order_for_reorder=[MinTemp@0 DESC], reverse_row_groups=true, dynamic_rg_pruning=eligible, pruning_predicate=MinTemp_null_count@0 > 0 OR MinTemp_null_count@0 != row_count@2 AND MinTemp_max@1 > <runtime>, required_guarantees=[]
          │     t1: DataSourceExec: file_groups={3 groups: [[/testdata/weather/result-000000.parquet:<int>..<int>], [/testdata/weather/result-000001.parquet:<int>..<int>, /testdata/weather/result-000002.parquet:<int>..<int>], [/testdata/weather/result-000002.parquet:<int>..<int>]]}, projection=[MinTemp], file_type=parquet, predicate=DynamicFilter [ MinTemp@0 IS NULL OR MinTemp@0 > <runtime> ], sort_order_for_reorder=[MinTemp@0 DESC], reverse_row_groups=true, dynamic_rg_pruning=eligible, pruning_predicate=MinTemp_null_count@0 > 0 OR MinTemp_null_count@0 != row_count@2 AND MinTemp_max@1 > <runtime>, required_guarantees=[]
          └──────────────────────────────────────────────────
        "));
        Ok(())
    }
